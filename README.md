...
2. 填写**检测仪能访问的 HA 地址**，例如 `http://192.168.1.10:8123`。Docker 请使用宿主机的局域网 IP 和映射端口。
3. 下一页会显示专属 URL。复制它并点击**提交**，提交后接收端才生效。
4. 把完整 URL 填入检测仪的自定义上传地址。收到第一条完整有效报文后，集成自动绑定其中的 `did`，并显示数值。
5. 以后在 **设置 → 设备与服务 → Bohu Air Quality → 对应设备的配置** 中，可再次复制 URL、确认气体单位、调整离线超时。

每次添加集成都会创建一个独立设备。不同设备使用不同 URL；同一个硬件 ID 不能重复绑定。URL 和绑定的 ID 会由 HA 保存，重启不变。更换物理设备时删除原集成条目再添加，会生成新 URL。删除设备后旧 URL 不再更新任何实体。

HA 地址会尽量预填，但容器内自动发现的 IP 可能无法被检测仪访问，请核对。此地址仅用于生成可复制的 URL，不会改变 HA 自身的监听端口或网络配置。
...
气体单位默认为**未确认**，不擅自标注为 mg/m³。请按设备屏幕或说明书选择 `mg/m³`、`µg/m³`、`ppm` 或 `ppb`。三项气体读数使用同一单位，数值原样保留，**选择单位不会换算数值**。该版本只适用于三项气体使用相同单位的设备。
...
已根据一台伯虎 ESP8266 + ARM 双 MCU 检测仪的实际上传报文实现。尚未验证其他型号、其他固件版本或下行控制。设备必须支持完整 URL 的自定义上传；本集成不修改固件、不执行 OTA，也不验证传感器测量精度。
...
- **不知道气体单位**：保留“未确认”，先检查设备屏幕或说明书。不要根据数值大小猜单位。
